# Capability: ML Subtitle Not Found

## Purpose

Defines how the ML service handles the case where no subtitle results are found for a requested title/episode, replacing an untyped `IndexError` propagation with a structured `SubtitleNotFoundError` that surfaces as HTTP 404.
